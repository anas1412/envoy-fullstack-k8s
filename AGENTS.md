...
- Prometheus runs with `--web.route-prefix=/prometheus` + `--web.external-url=http://localhost:30080/prometheus`. The prefix must match the Envoy route — if it drifts, the UI breaks (asset 404s / wrong links). The k8s readiness probes are `tcpSocket` on purpose (path-independent).
